...
const DailyTasks = () => {
  ...
  const completedTask = (id) => {
    const taskToComplete = dailyTasks.find((task) => task.id === id);
    if (taskToComplete) {
  ...
  const confirmDelete = () => {
    ...
    // 1️⃣ dailyTasks dan o‘chiramiz
    setDailyTasks((prev) =>
      prev.filter((task) => task.id !== taskToConfirm.id),
    );

    // 2️⃣ completedDailyTasks ga qo‘shamiz
    setCompletedDailyTasks((prev) => [...prev, taskToConfirm]);
  ...
  const deleteTask = (id) => {
    const taskToDelete = completedDailyTasks.find((task) => task.id === id);

    if (taskToDelete) {
      const updatedCompletedDailyTasks = completedDailyTasks.filter(
        (task) => task.id !== id,
      );

      setCompletedDailyTasks(updatedCompletedDailyTasks);
      localStorage.setItem(
        "completedDailyTasks",
        JSON.stringify(updatedCompletedDailyTasks),
      );
    }
  };
  ...
  return (
    <main className="px-3.5 py-5 rounded-md flex flex-col items-center gap-5 mt-28 w-full h-screen">
      ...
      <ul
        ...
        className="flex flex-col gap-3 w-full mt-4 overflow-y-scroll h-[58/vh]"
      >
